...
class MelDataset(torch.utils.data.Dataset):
    def __init__(self, training_files, segment_size, n_fft, num_mels,
                 ...
                 device=None, fmax_loss=None, fine_tuning=False, base_mels_path=None, train_from_npy=False):
        ...
        self.train_from_npy = train_from_npy
    ...
    def __getitem__(self, index):
        ...
        if self.train_from_npy:
            # Load mel directly from the .npy file
            mel = np.load(
            ...
            mel_loss = mel  # Use the loaded mel as both input and loss mel for consistency in training
        else:
            if not self.fine_tuning:
                if self.split:
                    if audio.size(1) >= self.segment_size:
